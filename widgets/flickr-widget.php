	...
	function widget($args, $instance)
	{
		...
		if($screen_name && $number) {
			...
			if($person->user->id) {
				...
				<ul class='flickr'>
					...
					<li class='flickr-photo'>
						<a href='<?php echo $photos_url->user->url; ?><?php echo $photo['id']; ?>' target='_blank'>
							<img src='<?php $url = "http://farm" . $photo['farm'] . ".static.flickr.com/" . $photo['server'] . "/" . $photo['id'] . "_" . $photo['secret'] . '_s' . ".jpg"; echo $url; ?>' alt='<?php echo $photo['title']; ?>' width="52" height="52" />
						</a>
					</li>
					<?php endforeach; ?>
				</ul>
				...
				echo '<p>Nome de usuário (flickr) inválido.</p>';
			}
		}
